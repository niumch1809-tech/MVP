...
export async function parseBomFileInBrowser(input: BrowserParseInput): Promise<BomFileRecord> {
...
function resolveQuoteName(input: BrowserParseInput, parsedSheet: ParsedSheet, hasMultipleSheets: boolean): string {
...
function parseWideSupplierRows(input: BrowserParseInput, parsedSheet: ParsedSheet, hasMultipleSheets = false): CanonicalBomRow[] {
  const fields = parsedSheet.fieldMapping;
  const supplierColumns = parsedSheet.headers.filter(
  ...
  parsedSheet.rows.forEach((row, rowIndex) => {
    ...
    supplierColumns.forEach((supplierColumn) => {
      ...
      rows.push({
        ...
        originalFields: { ...row, 供应商列: supplierColumn, 供应商报价: rawPrice },
        raw: row
      });
    });
  });
...
function readSheet(workbook: XLSX.WorkBook, sheetName: string, sheetIndex: number): ParsedSheet {
  ...
  if (quoteTitle) warnings.push(`识别到报价标题：${quoteTitle}，已按“供应商-产品名-型号-颜色”拆分。`);
  if (Object.keys(fieldMapping).length === 0) warnings.push("未能识别标准 BOM 字段，请检查表头是否在第一张表中。");

  return { sheetName, sheetIndex, quoteTitle, headerRowIndex, headers, rows, fieldMapping, warnings };
}
